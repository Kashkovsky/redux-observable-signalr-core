import { createSignalRHub, signalrHubUnstarted as signalrHubCreated, signalrHubFailedToStart, signalrConnected, signalrDisconnected, signalrError, reconnectSignalRHub, startSignalRHub, signalrStarted, stopSignalRHub, signalrStopped } from "./actions";
...
const createHub$ = action$ => action$.pipe(filter(isActionOf(createSignalRHub)), mergeMap(action => {
...
const beforeStartHub$ = action$ => action$.pipe(filter(isActionOf(signalrHubCreated)), mergeMap(action => {
...
const stopHub$ = action$ => action$.pipe(filter(isActionOf(stopSignalRHub)), map(findHub), filter(x => !!x), mergeMap(hub => hub.stop().pipe(map(() => signalrStopped({ hubName: hub.hubName, url: hub.url })), catchError(() => EMPTY))));
export const createReconnect$ = action$ => action$.pipe(filter(isActionOf(signalrError)), groupBy(action => action.hubName), mergeMap(group => group.pipe(exhaustMapHubToAction(({ action, hub }) => isOnline().pipe(filter(() => hub.autoReconnect), switchMap(online => {
    if (!online) {
        return EMPTY;
    }
    return timer(0, 5000);
}), map(_ => reconnectSignalRHub({ hubName: action.hubName, url: action.url })), takeUntil(action$.pipe(filter(isActionOf(signalrConnected)), ofHub(action))))))));
